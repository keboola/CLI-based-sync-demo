    ...
    def _create_text(diff_file):
        ...
        for event in diff:
            if event['event'] == 'ADD_BUCKET':
                table_data.append([f"[{event['bucket']['id']}]", 'Bucket added', '-'])

            elif event['event'] == 'DROP_BUCKET':
                table_data.append([f"[{event['bucket']['id']}]", 'Bucket removed', '-'])

            elif event['event'] == 'MODIFY_BUCKET':
                table_data.append([f"[{event['bucket']['id']}]", 'Bucket modified', '-'])

            elif event['event'] == 'ADD_TABLE':
                table_data.append([f"[{event['table']['id']}]", 'Table added', '-'])

            elif event['event'] == 'DROP_TABLE':
                table_data.append([f"[{event['table']['id']}]", 'Table removed', '-'])
